...
def load_checkpoint(model, chkp_file, model_device=None, strict=True, lean=False, optimizer=None):
    ...
    use_ema = True
    if use_ema:
        model_weight = checkpoint['model_ema']
    else:
        model_weight = checkpoint['state_dict']
    
    anomalous_keys = model.load_state_dict(model_weight, strict)
    ...
    if banks is not None:
        ...
        if hasattr(model, 'module'):
            ...
            model.module.banks = banks
        else:
            model.banks = banks

        # print('bank before resume', model.module.banks)
