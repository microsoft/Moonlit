...
class RegNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for block in self.trunk_output:
            print(x.shape)
            x =  block(x)
        print(x.shape)
        x = self.avgpool(x)
    ...
    def forward_to_stage(self, x, stage_name):
        assert stage_name in ['stage1', 'stage2'] # TODO resnet series only have 4 stages, all with stride 2
        prev_x = x
        x = self.stem(x)
        for idx, block in enumerate(self.trunk_output, start=1):
            prev_x = x
            x = block(x)
            if idx == int(stage_name[-1]):
                break
        return x, prev_x

    def get_cin(self, stage_name):
        stage_idx = int(stage_name[-1])
        if stage_idx == 1:
            return self.stem[0].out_channels
        else:
            return self.block_params.widths[stage_idx - 1 - 1]

    def get_cout(self, stage_name):
        stage_idx = int(stage_name[-1])
        return self.block_params.widths[stage_idx - 1]
