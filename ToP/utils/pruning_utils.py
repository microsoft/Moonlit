...
def load_model_with_zs(model_path, model_class, zs=None, token_prune_loc=None):
    config_path = os.path.join(model_path, "config.json")
    if os.path.exists(config_path):
        config = AutoConfig.from_pretrained(model_path)
    model = model_class.from_pretrained(model_path, config=config, token_prune_loc=token_prune_loc)
...
def do_token_pruning(model, zs):
    if "token_z" not in zs:
        print("no token pruning is performed before")
        return
    bert = model.bert if hasattr(model, "bert") else model.roberta
    bert.encoder.hard_token_mask = ((zs["token_z"] > 0.0) * 1.0).cuda()
    bert.encoder.hard_pruner_mask = ((zs["pruner_z"] > 0.0) * 1.0).cuda()
    print("token pruning mask is hardcoded into model. IT SHOULD NOT HAPPEN FOR PRUNING STAGE!!!")
    print("="*80)
    for z in bert.encoder.hard_token_mask:
        print(get_z_str(z))
    print(bert.encoder.hard_pruner_mask)
    print("="*80)
    

def load_model(model_path, model_class, zs=None, token_prune_loc=None):
    assert zs is not None
    model = load_model_with_zs(model_path, model_class, zs, token_prune_loc=token_prune_loc)
...
def prune_model_with_z(zs, model):
    ...
    if "head_z" in zs:
        ...
        for layer in range(len(head_z)):
            ...
            # TODO: disable layer pruning info printing
    ...
    # TODO: disable layer info printing
    # for layer in range(0, 12):
    #     print("Layer:", layer)
    #     if bert.encoder.layer[layer].attention.self.query is not None:
...
def load_pruned_model(model, weights):
    ...
    architecture = config.architectures[0].lower() if config.architectures is not None else config._name_or_path
